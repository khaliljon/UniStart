  ...
  const handleSubmit = async (e: React.FormEvent, publish: boolean = false) => {
    ...
    try {
      ...
      for (let i = 0; i < flashcards.length; i++) {
        ...
        if (card.id) {
          await api.put(`/flashcards/${card.id}`, cardData);
        } else {
          await api.post('/flashcards', cardData);
        }
      }
      ...
      alert(error.response?.data?.message || 'Не удалось обновить набор');
    } finally {
      setSaving(false);
    }
  };
