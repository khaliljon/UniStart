...
namespace UniStart.Data
{
    public class ApplicationDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // ”никальность: один пользователь - одна запись прогресса по карточке
            modelBuilder.Entity<UserFlashcardProgress>()
            ...
            // ”никальность: один пользователь - одна запись доступа к набору
            modelBuilder.Entity<UserFlashcardSetAccess>()
            ...
            modelBuilder.Entity<UserQuizAnswer>()
                ...
                .OnDelete(DeleteBehavior.Restrict); // Ќе удал€ем вопрос, если есть ответы

            modelBuilder.Entity<UserQuizAnswer>()
                .HasOne(ua => ua.SelectedAnswer)
                .WithMany()
                .HasForeignKey(ua => ua.SelectedAnswerId)
                .OnDelete(DeleteBehavior.Restrict); // Ќе удал€ем ответ, если есть выбор пользовател€
            ...
            // Indexes
            // Flashcard.NextReviewDate больше не используетс€ (перемещено в UserFlashcardProgress)
            // ќставл€ем дл€ обратной совместимости, но не индексируем
            ...
            modelBuilder.Entity<SystemSettings>()
                .HasData(new SystemSettings
                {
                    Id = 1,
                    SiteName = "UniStart",
                    SiteDescription = "ќбразовательна€ платформа дл€ изучени€ с помощью карточек и тестов",
